"""Tests for channel_distinguishability."""

import pytest

from toqito.channel_metrics import channel_distinguishability
from toqito.channel_ops import kraus_to_choi
from toqito.channels import amplitude_damping, dephasing, depolarizing

# creating two amplitude damping channels.
amp_damp_1 = kraus_to_choi(amplitude_damping(gamma=0.22))
# In Kraus representation.
amp_damp_1_kraus = amplitude_damping(gamma=0.22)
amp_damp_2 = kraus_to_choi(amplitude_damping(gamma=0.35))


@pytest.mark.parametrize(
    "test_input_1, test_input_2, prior_prob, expected",
    [
        # Distinguishing two identical channels.
        (dephasing(2), dephasing(2), [0.5, 0.5], 0.5),
        # Distinguishing two amplitude damping channels.
        (amp_damp_1, amp_damp_2, [0.2, 0.8], 0.8),
        # One channel in Kraus and another in Choi representation.
        (amp_damp_1_kraus, amp_damp_2, [0.2, 0.8], 0.8),
    ],
)
def test_channel_distinguishability_valid_inputs(test_input_1, test_input_2, prior_prob, expected):
    """Test function works as expected for valid inputs."""
    calculated_value = channel_distinguishability(test_input_1, test_input_2, prior_prob)
    assert pytest.approx(expected, 1e-3) == calculated_value


@pytest.mark.parametrize(
    "test_input1, test_input_2, prior_prob, expected_msg",
    [
        # Inconsistent dimensions between two channels.
        (
            depolarizing(4),
            dephasing(2),
            [0.5, 0.5],
            "The channels must have the same dimension input and output spaces as each other.",
        ),
        # Sum of prior probabilities greater than 1.
        (
            dephasing(2),
            dephasing(2),
            [0.5, 0.9],
            "Sum of prior probabilities must add up to 1.",
        ),
        # Length of prior probability list not equal to two.
        (
            dephasing(2),
            dephasing(2),
            [0.5],
            "p must be a probability distribution with 2 entries.",
        ),
    ],
)
def test_channel_distinguishability_invalid_inputs(test_input1, test_input_2, prior_prob, expected_msg):
    """Test function raises error as expected for invalid inputs."""
    with pytest.raises(ValueError, match=expected_msg):
        channel_distinguishability(test_input1, test_input_2, prior_prob)
