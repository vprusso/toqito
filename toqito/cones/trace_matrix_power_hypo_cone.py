"""CVXPY constraints for the hypograph of the matrix-power trace."""
...
def trace_matrix_power_hypo_cone(
    ...
    *,
    ...
    r"""Return CVXPY constraints for the hypograph of \(\operatorname{tr}(C A^{p})\).

    For ``power`` \(p \in [0, 1]\) the map \(A \mapsto \operatorname{tr}(C A^{p})\)
    is concave in \(A\) (with fixed PSD weight \(C\)). The constraints enforce

    \[
        t \leqslant \operatorname{tr}(C A^{p})
    \]
    ...
    \[
        G_{p}(I, A) \succeq T,
        \qquad
        t \leqslant \operatorname{tr}(C\, T)
    \]

    using the restricted hypograph (`fullhyp=False`) [@fawzi2015matrixgeometric].
    ...
    Args:
        ...
        power: The matrix power \(p \in [0, 1]\).
    ...
    Examples:
        ...
        import cvxpy
        import numpy as np
        from toqito.cones import trace_matrix_power_hypo_cone
