...
def test_lieb_ando_epi_cone_at_constant(dim: int, power: float, hermitian: bool):
    """Minimize ``t`` at fixed Constants and compare to the numeric Lieb--Ando value."""
    ...
    np.testing.assert_allclose(float(cvx_val), ref, rtol=5e-2, atol=2e-2)


def test_lieb_ando_epi_cone_identity_k():
    """Identity ``K`` minimizes toward the numeric reference on a diagonal pair."""
...
def test_lieb_ando_epi_cone_composition():
    """Free symmetric ``A`` with fixed ``B`` and ``K``; minimize Lieb--Ando."""
    n = 2
    eps = 0.25
    mat_b = np.eye(n)
    mat_k = np.eye(n)
...
def test_lieb_ando_epi_cone_mat_a_not_square() -> None:
    ...
    with pytest.raises(ValueError, match=re.escape("mat_a must be square.")):
        lieb_ando_epi_cone(mat_a, mat_b, np.ones((2, 3)), t, 1.5)


def test_lieb_ando_epi_cone_mat_b_not_square() -> None:
    ...
    with pytest.raises(ValueError, match=re.escape("mat_b must be square.")):
        lieb_ando_epi_cone(mat_a, mat_b, np.ones((2, 2)), t, 1.5)


def test_lieb_ando_epi_cone_power_invalid() -> None:
    ...
    with pytest.raises(
        ...
        lieb_ando_epi_cone(mat_a, mat_b, np.eye(2), t, 0.5)


def test_lieb_ando_epi_cone_mat_k_wrong_type() -> None:
    """Reject non-numpy ``mat_k``."""
    mat_a = cvxpy.Variable((2, 2), symmetric=True)
    mat_b = cvxpy.Variable((2, 2), symmetric=True)
    t = cvxpy.Variable()
    with pytest.raises(ValueError, match=re.escape("mat_k must be a numpy array")):
        lieb_ando_epi_cone(mat_a, mat_b, cvxpy.Constant(np.eye(2)), t, 1.5)


def test_lieb_ando_epi_cone_mat_k_shape_mismatch() -> None:
    ...
    with pytest.raises(
        ...
        lieb_ando_epi_cone(mat_a, mat_b, np.eye(3), t, 1.5)
