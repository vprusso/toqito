"""Tests for trace_matrix_power_hypo_cone."""
...
def test_trace_matrix_power_hypo_cone_at_constant(dim: int, power: float, hermitian: bool):
    ...
    np.testing.assert_allclose(float(cvx_val), ref, rtol=1e-3, atol=5e-3)


def test_trace_matrix_power_hypo_cone_default_mat_c_is_identity():
...
def test_trace_matrix_power_hypo_cone_composition():
    """Free ``Variable`` with ``eps I ⪯ A ⪯ I``; maximize ``tr(A^{1/2})``."""
    n = 2
    eps = 0.2
...
def test_trace_matrix_power_hypo_cone_mat_a_not_square() -> None:
    """Reject non-square ``mat_a``."""
    mat_a = cvxpy.Variable((2, 3))
    t = cvxpy.Variable()
    with pytest.raises(ValueError, match=re.escape("mat_a must be square.")):
        trace_matrix_power_hypo_cone(mat_a, t, 0.5)


def test_trace_matrix_power_hypo_cone_power_invalid() -> None:
...
def test_trace_matrix_power_hypo_cone_mat_c_wrong_type() -> None:
    """Reject non-numpy ``mat_c``."""
    mat_a = cvxpy.Variable((2, 2), symmetric=True)
    t = cvxpy.Variable()
    with pytest.raises(ValueError, match=re.escape("mat_c must be a numpy array")):
        trace_matrix_power_hypo_cone(mat_a, t, 0.5, mat_c=cvxpy.Constant(np.eye(2)))


def test_trace_matrix_power_hypo_cone_mat_c_not_psd() -> None:
    ...
    with pytest.raises(
        ...
        trace_matrix_power_hypo_cone(mat_a, t, 0.5, mat_c=np.diag([1.0, -0.5]))


def test_trace_matrix_power_hypo_cone_shape_mismatch() -> None:
    ...
    with pytest.raises(
        ...
        trace_matrix_power_hypo_cone(mat_a, t, 0.5, mat_c=np.eye(3))
