"""Test state_exclusion with measurement='locc' (one-way LOCC see-saw)."""
...
def _locc(states, **kwargs):
    """Run the one-way LOCC exclusion path and return just the error value."""
    return state_exclusion(states, measurement="locc", **kwargs)[0]
...
def test_product_orthogonal_states_match_global():
    ...
    val = _locc(states, dimensions=[2, 2], reps=3, seed=0)
...
def test_locc_never_below_global():
    ...
    locc = _locc(states, dimensions=[2, 2], reps=3, seed=1)
...
def test_reproducible_with_seed():
    ...
    states = [_dm(v) for v in _GAP_VECS]
    first = _locc(states, dimensions=[2, 2], reps=2, seed=7)
    second = _locc(states, dimensions=[2, 2], reps=2, seed=7)
    np.testing.assert_allclose(first, second, atol=1e-9)


def test_returns_measurement_operators():
    """The locc path returns the induced global measurement operators that sum to the identity."""
    e_0, e_1 = np.array([1, 0]), np.array([0, 1])
    states = [_dm(np.kron(e_0, e_0)), _dm(np.kron(e_1, e_1)), _dm(np.kron(e_0, e_1))]
    _, measurements = state_exclusion(states, measurement="locc", dimensions=[2, 2], reps=2, seed=0)
    np.testing.assert_allclose(sum(measurements), np.identity(4), atol=1e-4)
...
def test_domino_states_are_locc_excludable():
    ...
    val = _locc(states, dimensions=[3, 3], reps=2, seed=0)
    np.testing.assert_allclose(val, 0.0, atol=1e-4)


def test_requires_dimensions():
    """Omitting `dimensions` for the locc measurement raises a ValueError."""
    states = [_dm((1, 0, 0, 0)), _dm((0, 0, 0, 1))]
    with pytest.raises(ValueError, match="dimensions"):
        state_exclusion(states, measurement="locc")


def test_locc_rejects_unambiguous():
    """The locc measurement only supports the min_error strategy."""
    states = [_dm((1, 0, 0, 0)), _dm((0, 0, 0, 1))]
    with pytest.raises(ValueError, match="min_error"):
        state_exclusion(states, measurement="locc", strategy="unambiguous", dimensions=[2, 2])


def test_dimensions_product_must_match_states():
    ...
    with pytest.raises(ValueError, match="product"):
        _locc(states, dimensions=[2, 3])  # 2 * 3 = 6 != 4
...
def test_runs_without_seed():
    ...
    e_0, e_1 = np.array([1, 0]), np.array([0, 1])
    states = [_dm(np.kron(e_0, e_0)), _dm(np.kron(e_1, e_1)), _dm(np.kron(e_0, e_1))]
    val = _locc(states, dimensions=[2, 2], reps=1)
    np.testing.assert_allclose(val, 0.0, atol=1e-4)
...
def test_custom_num_alice_outcomes():
    ...
    states = [_dm(v) for v in _GAP_VECS]
    val = _locc(states, dimensions=[2, 2], num_alice_outcomes=4, reps=2, seed=0)
