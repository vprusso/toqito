"""Tests for Bell inequality inequality_max (General and Qubits)."""
...
from toqito.state_opt.bell_inequality_max import bell_inequality_max_general, bell_inequality_max_qubits
...
def test_chsh_fc_classical(chsh_fc, desc_chsh):
    ...
    assert bell_inequality_max_general(chsh_fc, desc_chsh, "fc", "classical") == pytest.approx(2.0, abs=ATOL)
...
def test_chsh_fc_quantum(chsh_fc, desc_chsh):
    ...
    assert bell_inequality_max_general(chsh_fc, desc_chsh, "fc", "quantum", tol=1e-7) == pytest.approx(
...
def test_chsh_fc_nosignal(chsh_fc, desc_chsh):
    ...
    assert bell_inequality_max_general(chsh_fc, desc_chsh, "fc", "nosignal", tol=1e-9) == pytest.approx(4.0, abs=5e-5)


def test_chsh_cg_classical(chsh_cg, desc_chsh):
    """Test classical maximum for CHSH inequality in CG notation."""
    assert bell_inequality_max_general(chsh_cg, desc_chsh, "cg", "classical") == pytest.approx(0.0, abs=ATOL)
...
def test_chsh_cg_quantum(chsh_cg, desc_chsh):
    ...
    assert bell_inequality_max_general(chsh_cg, desc_chsh, "cg", "quantum", tol=1e-7) == pytest.approx(
        expected, rel=RTOL, abs=ATOL
    )
...
def test_chsh_cg_nosignal(chsh_cg, desc_chsh):
    ...
    assert bell_inequality_max_general(chsh_cg, desc_chsh, "cg", "nosignal", tol=1e-9) == pytest.approx(0.5, abs=ATOL)
...
def test_chsh_game_fp_classical(chsh_game_fp, desc_chsh):
    ...
    assert bell_inequality_max_general(chsh_game_fp, desc_chsh, "fp", "classical") == pytest.approx(0.75, abs=ATOL)
...
def test_chsh_game_fp_quantum(chsh_game_fp, desc_chsh):
    ...
    assert bell_inequality_max_general(chsh_game_fp, desc_chsh, "fp", "quantum", k="1+ab", tol=1e-7) == pytest.approx(
        expected, rel=RTOL, abs=ATOL
    )
...
def test_chsh_game_fp_nosignal(chsh_game_fp, desc_chsh):
    ...
    assert (
    bell_inequality_max_general(chsh_game_fp, desc_chsh, "fp", "nosignal", tol=1e-9)
...
def test_i3322_cg_classical(i3322_cg, desc_i3322):
    ...
    assert bell_inequality_max_general(i3322_cg, desc_i3322, "cg", "classical") == pytest.approx(1.0, abs=ATOL)
...
def test_i3322_cg_quantum(i3322_cg, desc_i3322, k_level, expected_val_toqito):
    ...
    sdp_tol = 1e-7
    assert bell_inequality_max_general(i3322_cg, desc_i3322, "cg", "quantum", k=k_level, tol=sdp_tol) == pytest.approx(
...
def test_i3322_cg_nosignal(i3322_cg, desc_i3322):
    ...
    assert bell_inequality_max_general(i3322_cg, desc_i3322, "cg", "nosignal", tol=1e-9) == pytest.approx(2.0, abs=ATOL)
...
def test_classical_swap_fc():
    ...
    val_no_swap = bell_inequality_max_general(dummy_fc_32, desc_32, "fc", "classical")

    desc_23 = [2, 2, 2, 3]
    dummy_fc_23 = dummy_fc_32.T
    val_swap = bell_inequality_max_general(dummy_fc_23, desc_23, "fc", "classical")
...
def test_classical_swap_fp():
    ...
    val_swap = bell_inequality_max_general(dummy_fp_swap, desc_swap, "fp", "classical")

    desc_noswap = [3, 2, 1, 2]
    dummy_fp_noswap = np.transpose(dummy_fp_swap, (1, 0, 3, 2))
    val_noswap = bell_inequality_max_general(dummy_fp_noswap, desc_noswap, "fp", "classical")
...
def test_invalid_notation(chsh_fc, desc_chsh):
    ...
    with pytest.raises(ValueError, match="Invalid notation"):
        bell_inequality_max_general(chsh_fc, desc_chsh, "invalid", "classical")
...
def test_invalid_mtype(chsh_fc, desc_chsh):
    ...
    with pytest.raises(ValueError, match="Invalid mtype"):
        bell_inequality_max_general(chsh_fc, desc_chsh, "fc", "invalid_type")
...
def test_classical_nonbinary_fc_input_error():
    ...
    with pytest.raises(ValueError, match="Notation conversion failed for non-binary scenario"):
        bell_inequality_max_general(dummy_fc, desc, "fc", "classical")
...
def test_classical_no_bob_inputs():
    ...
    val_fc = bell_inequality_max_general(coeffs_fc, desc_2x2_mb0, "fc", "classical")
    ...
    val_fp = bell_inequality_max_general(coeffs_fp_gen, desc_gen_mb0, "fp", "classical")
...
def test_classical_no_alice_inputs():
    ...
    coeffs_fc = np.array([[0, 1, -1]])
    val_fc = bell_inequality_max_general(coeffs_fc, desc_2x2_ma0, "fc", "classical")
    expected_fc = coeffs_fc[0, 0] + np.sum(np.abs(coeffs_fc[0, 1:]))
    assert val_fc == pytest.approx(expected_fc, abs=ATOL)

    desc_gen_ma0 = [2, 3, 0, 2]
    coeffs_fp_gen = np.zeros((2, 3, 0, 2))
    val_fp = bell_inequality_max_general(coeffs_fp_gen, desc_gen_ma0, "fp", "classical")
    assert val_fp == pytest.approx(0.0, abs=ATOL)
...
def test_classical_binary_ma0_fp():
    ...
    assert bell_inequality_max_general(coeffs_fp, desc_ma0, "fp", "classical") == pytest.approx(0.0, abs=ATOL)
...
def test_classical_binary_mb0_cg():
    ...
    assert bell_inequality_max_general(coeffs_cg, desc_mb0, "cg", "classical") == pytest.approx(1.0, abs=ATOL)
...
def test_nosignal_oa1():
    ...
    assert bell_inequality_max_general(coeffs_cg, desc, "cg", "nosignal", tol=1e-9) == pytest.approx(2.0, abs=ATOL)
...
def test_nosignal_fc_nonbinary_error():
    ...
    with pytest.raises(ValueError, match="Notation conversion failed: 'fc' notation is only supported"):
        bell_inequality_max_general(dummy_fc, desc, "fc", "nosignal")
...
def test_quantum_fc_nonbinary_error():
    ...
    dummy_fc = np.array([[0, 0, 0], [0, 1, 1], [0, 1, -1]])
    with pytest.raises(ValueError, match="Notation conversion failed: 'fc' notation is only supported"):
        bell_inequality_max_general(dummy_fc, desc, "fc", "quantum")
...
def test_quantum_invalid_k_error(chsh_cg, desc_chsh):
    ...
    with pytest.raises(ValueError, match="Invalid NPA level k=-1"):
        bell_inequality_max_general(chsh_cg, desc_chsh, "cg", "quantum", k=-1)
    with pytest.raises(ValueError, match=r"Invalid NPA level k='invalid_level'"):
        bell_inequality_max_general(chsh_cg, desc_chsh, "cg", "quantum", k="invalid_level")
    with pytest.raises(ValueError, match=r"Invalid NPA level k='1\+abc'"):
        bell_inequality_max_general(chsh_cg, desc_chsh, "cg", "quantum", k="1+abc")
...
def test_classical_cg_conversion_error(desc_chsh):
    ...
    with pytest.raises(ValueError, match="Notation conversion failed for binary scenario: CG coefficient shape"):
        bell_inequality_max_general(invalid_coeffs_cg, desc_chsh, "cg", "classical")
...
def test_classical_nonbinary_cg_conversion_error():
    ...
    with pytest.raises(ValueError, match="Notation conversion failed for non-binary scenario: CG coefficient shape"):
        bell_inequality_max_general(invalid_coeffs_cg, desc_nonbin, "cg", "classical")
...
def test_classical_binary_fp_conversion_error(desc_chsh):
    ...
    with pytest.raises(ValueError, match="Notation conversion failed for binary scenario: FP coefficient shape"):
        bell_inequality_max_general(invalid_coeffs_fp, desc_chsh, "fp", "classical")
...
def test_classical_nonbinary_ob1():
    ...
    coeffs_fp[0, 0, 0, 0] = 1
    coeffs_fp[0, 0, 1, 1] = 1
    assert bell_inequality_max_general(coeffs_fp, desc, "fp", "classical") == pytest.approx(2.0, abs=ATOL)

    coeffs_fp_2 = np.zeros((2, 1, 2, 2))
    coeffs_fp_2[0, 0, 0, 0] = 0.8
    coeffs_fp_2[1, 0, 0, 0] = 0.2
    coeffs_fp_2[0, 0, 1, 1] = 1.0
    assert bell_inequality_max_general(coeffs_fp_2, desc, "fp", "classical") == pytest.approx(1.8, abs=ATOL)
...
def test_classical_empty_fp():
    ...
    assert bell_inequality_max_general(coeffs_fp_ma0, desc_ma0, "fp", "classical") == pytest.approx(0.0, abs=ATOL)

    desc_mb0 = [3, 2, 2, 0]
    coeffs_fp_mb0 = np.zeros((3, 2, 2, 0))
    assert bell_inequality_max_general(coeffs_fp_mb0, desc_mb0, "fp", "classical") == pytest.approx(0.0, abs=ATOL)
...
def test_classical_nonbinary_fp_dense():
    ...
    sum_ab[sum_ab == 0] = 1
    coeffs_fp /= sum_ab
    result = bell_inequality_max_general(coeffs_fp, desc_nonbin, "fp", "classical")
...
def test_nosignal_cg_shape_error(desc_chsh):
    ...
    with pytest.raises(ValueError, match="Coefficient shape"):
        bell_inequality_max_general(invalid_coeffs_cg, desc_chsh, "cg", "nosignal")
...
def test_quantum_cg_shape_error(desc_chsh):
    ...
    invalid_coeffs_cg = np.zeros((3, 4))  # Correct shape is (3, 3)
    with pytest.raises(ValueError, match="Coefficient shape"):
        bell_inequality_max_general(invalid_coeffs_cg, desc_chsh, "cg", "quantum")
...
def test_classical_fc_shape_error(desc_chsh):
    ...
    with pytest.raises(ValueError, match="Notation conversion failed for binary scenario: FC coefficient shape"):
        bell_inequality_max_general(invalid_coeffs_fc, desc_chsh, "fc", "classical")
...
def test_classical_nonbinary_fp_shape_error():
    ...
    with pytest.raises(ValueError, match="Notation conversion failed for non-binary scenario: FP coefficient shape"):
        bell_inequality_max_general(invalid_coeffs_fp, desc_nonbin, "fp", "classical")
...
def test_classical_nonbinary_swap_triggered():
    ...
    sum_ab[sum_ab == 0] = 1
    coeffs_fp /= sum_ab
    result = bell_inequality_max_general(coeffs_fp, desc, "fp", "classical")
    assert isinstance(result, float)
...
def test_nosignal_infeasible_status(capfd):
    ...
    with patch("cvxpy.Problem", return_value=mock_problem):
        with patch("toqito.state_opt.bell_inequality_max.fp_to_cg", side_effect=None):
            desc = [2, 2, 1, 1]
            coeffs_cg = np.array([[0, 0], [0, 1]])
            result = bell_inequality_max_general(coeffs_cg, desc, "cg", "nosignal")
...
def test_quantum_infeasible_status(capfd):
    ...
    with patch("cvxpy.Problem", return_value=mock_problem):
        # Clean string patching
        with patch("toqito.state_opt.bell_inequality_max.bell_npa_constraints", return_value=[]):
            desc = [2, 2, 2, 2]
            coeffs_cg = np.array([[0, -1, 0], [-1, 1, 1], [0, 1, -1]])
            result = bell_inequality_max_general(coeffs_cg, desc, "cg", "quantum")
            captured = capfd.readouterr()
            assert result == -np.inf
...
def test_quantum_npa_error(chsh_cg, desc_chsh):
    ...
    with patch("toqito.state_opt.bell_inequality_max.bell_npa_constraints", side_effect=ValueError("Mock NPA Error")):
...
def test_quantum_result_is_nan(chsh_cg, desc_chsh):
    ...
    with patch("cvxpy.Problem", return_value=mock_problem):
        with patch("toqito.state_opt.bell_inequality_max.bell_npa_constraints", return_value=[]):
            assert bell_inequality_max_general(chsh_cg, desc_chsh, "cg", "quantum") == -np.inf
...
def test_nosignal_result_is_nan(chsh_cg, desc_chsh):
    ...
    with patch("cvxpy.Problem", return_value=mock_problem):
        assert bell_inequality_max_general(chsh_cg, desc_chsh, "cg", "nosignal") == -np.inf
...
def test_nosignal_fp_conversion_internal_error(desc_chsh):
    ...
    with patch("toqito.state_opt.bell_inequality_max.fp_to_cg", side_effect=ValueError("Internal fp_to_cg error")):
        with pytest.raises(ValueError, match="Notation conversion failed: Internal fp_to_cg error"):
            bell_inequality_max_general(dummy_fp_coeffs, desc_chsh, "fp", "nosignal")
...
def test_quantum_fc_conversion_internal_error(chsh_fc, desc_chsh):
    ...
    with patch("toqito.state_opt.bell_inequality_max.fc_to_cg", side_effect=ValueError("Internal fc_to_cg error")):
        with pytest.raises(ValueError, match="Notation conversion failed: Internal fc_to_cg error"):
            bell_inequality_max_general(chsh_fc, desc_chsh, "fc", "quantum")
...
def test_classical_nonbinary_cg_to_fp_internal_error():
    ...
    with patch("toqito.state_opt.bell_inequality_max.cg_to_fp", side_effect=ValueError("Internal cg_to_fp error")):
        with pytest.raises(
            ValueError, match="Notation conversion failed for non-binary scenario: Internal cg_to_fp error"
        ):
            bell_inequality_max_general(coeffs_cg_actual_nonbin, desc_actual_nonbin, "cg", "classical")
