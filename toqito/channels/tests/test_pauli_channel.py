...
def test_pauli_channel_kraus_operators(prob):
    ...
    _, kraus_operators = pauli_channel(prob=prob, kraus_ops=True)
...
def test_pauli_channel_input_matrix_properties(input_mat, prob):
    ...
    Phi, output_mat, kraus_ops = pauli_channel(prob=prob, input_mat=input_mat, kraus_ops=True)
...
def test_pauli_channel_choi_matrix_properties(num_qubits):
    ...
    """
    Phi, _ = pauli_channel(prob=num_qubits, kraus_ops=True)
    Phi = np.array(Phi)
...
def test_pauli_channel_zero_probability(prob):
    ...
    """
    _, kraus_ops = pauli_channel(prob=prob, kraus_ops=True)
