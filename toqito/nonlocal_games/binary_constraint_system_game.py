...
def create_bcs_constraints(M: np.ndarray, b: np.ndarray) -> list[np.ndarray]:
    ...
    .. jupyter-execute::
          ...
          import numpy as np
          from nonlocal_games.binary_constraint_system_game import create_bcs_constraints
...
def generate_solution_group(M: np.ndarray, b: np.ndarray) -> tuple[list[int], list[int]]:
    ...
    .. jupyter-execute::
          ...
          import numpy as np
          from nonlocal_games.binary_constraint_system_game import generate_solution_group
...
def check_perfect_commuting_strategy(M: np.ndarray, b: np.ndarray) -> bool:
    ...
    ..jupyter-execute::
         ...
         import numpy as np
         from nonlocal_games.binary_constraint_system_game import check_perfect_commuting_strategy
         M = np.array([[1, 1], [1, 1]])
         b = np.array([0, 1])
