...
#     The quantum state $|\psi\rangle$ is then just a probability
#     distribution over the possible values of $\lambda$. A key
#     implication is that the distributions for two different quantum states,
#     say $|\psi_0\rangle$ and $|\psi_1\rangle$, could overlap. This
#     means that for some underlying physical states $\lambda$, the system
#     could have been prepared in *either* $|\psi_0\rangle$ or
#     $|\psi_1\rangle$.
#
...
# any specific physical model for $\lambda$; the plot is purely a visual aid to
# make the concept of overlapping probability distributions concrete.
#
# For this illustration, we represent the space of possible ontic states $\lambda$
...
# $p(\lambda | \psi_0)$ and $p(\lambda | \psi_1)$. The specific choice of Gaussian distributions
...
# %%
#  !!! note
#     The shaded region $\Delta$ represents the set of ontic states
#     $\lambda$ that are ambiguous—the system could have been prepared as
...
# $$
#
# If their underlying reality distributions overlap, it's possible to prepare a system where its true state $\lambda$ is consistent
...
# state $(\lambda_1, \lambda_2)$ is compatible with *any* of the
...
# $\{|\Psi_i\rangle\}$ is antidistinguishable if there exists a
# measurement with outcomes $\{M_i\}$ such that
# $\langle \Psi_i | M_i | \Psi_i \rangle = 0$ for all $i$.
#
# This leads to a contradiction:
#
# *   The epistemic model predicts that sometimes the underlying
#         reality $(\lambda_1, \lambda_2)$ is ambiguous.
# *   In these cases, the measurement (which only depends on
#         $\lambda$) must produce *some* outcome, say outcome $k$.
# *   But what if the state was *actually* prepared as
#         $|\Psi_k\rangle$? Quantum mechanics says outcome $k$
#         is impossible for this state.
#
# This contradiction implies that the initial assumption—that the
# distributions for $|0\rangle$ and $|+\rangle$ overlap—must
...
#
# We can now use `|toqito⟩` to verify the key requirement for the PBR
# theorem that the set of four states constructed from $|0\rangle$
# and $|+\rangle$ are indeed antidistinguishable.
...
# `|toqito⟩`'s  [`state_exclusion`][toqito.state_opt.state_exclusion.state_exclusion] function supports the theorem's
...
# %%
# The inner product of the two base states is $\cos(\theta)$.
# For these to be antidistinguishable, we need to check the condition from the paper.
# The theorem states that if $2^{1/n} - 1 < \tan(\theta/2)$, a contradiction is obtained.
# For $n=2$ and $\theta=\pi/3$, we have $\tan(\theta/2) = \tan(\pi/6) \approx 0.577$.
# The other side of the inequality is $2^{1/2} - 1 \approx 0.414$.
