...
from toqito.nonlocal_games.coin_flipping.weak_coin_flipping \
    import weak_coin_flipping
...
class TestWeakCoinFlipping(unittest.TestCase):
    ...
    def test_weak_coin_flipping(self):
        ...
        rho = np.kron(e_1 * e_1.conj().T, e_0 * e_0.conj().T) + \
            np.kron(e_m * e_m.conj().T, e_1 * e_1.conj().T)

        self.assertEqual(np.isclose(weak_coin_flipping(rho),
