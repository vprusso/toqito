...
\begin{document}
	...
	\begin{center}
		\begin{tikzpicture}[text depth=1pt, bend angle=45, line width = 5pt, scale=0.75]
		
				\tikzstyle {AliceBob} = [shape=rectangle, rounded corners, draw=black!50, thick, fill=black!20, minimum height=15mm, minimum width=15mm,
				 inner sep=1ex]
   		 		\tikzstyle {AliceLab} = [shape=circle, rounded corners, dotted, draw=blue!50, thick,fill=blue!35, minimum height=25mm, minimum width=25mm] 
   		 		\tikzstyle {BobLab} = [shape=circle, rounded corners, dotted, draw=purple!50, thick,fill=purple!35, minimum height=25mm, minimum width=25mm] 
				\tikzstyle {Charlie} = [shape=ellipse, rounded corners, dotted, draw=black!50, thick,fill=black!35, minimum height=15mm, minimum width=50mm] 

				\node[Charlie] (Charlie) at (5,0) {$\eta = \{ \rho_1, \cdots, \rho_k \}$ };

				\node[AliceBob] (Alice) at (0,-5) {Alice}; 
				\node[AliceBob] (Bob) at (10,-5) {Bob};
	
				\node[AliceLab] (AliceLab) at (-3,-8) {?};
				\node[BobLab] (BobLab) at (13,-8) {?};
		
				% Remove side things and remove psi from lab thing
				\draw[<->, dashed, orange] (Alice) -- node[above]{\textcolor{orange}{$b \in \{0,1\}^n$}} (Bob);
				\draw[->, line width = 3pt, draw=blue!50 ] (Charlie) -- node[above left] {} (Alice);
				\draw[->, line width = 3pt, draw=purple!50 ] (Charlie) -- node[above right] {} (Bob);
				\draw[<->, dashed, line width = 4pt, purple] (Alice) -- (AliceLab);	
				\draw[<->, dashed, line width = 4pt, purple] (Bob) -- (BobLab);			
		\end{tikzpicture}
	\end{center}		
\end{document}
