...
Static typing is enforced with :code:`ty <https://docs.astral.sh/ty/>`_. Before submitting a pull request, run the
type checker against the source tree (Ty lives in the ``lint`` dependency group):
